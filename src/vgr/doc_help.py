...
class VgrLexer(RegexLexer):
    ...
    tokens = {
        ...
        "root": [
            ...
            (r"(?:(?<=^)|(?<=[\s:;,]))(Batch|Template|Select|Ldap|Search|Connect|User|Password|Base|Filter|Attributes|Scope|Are|Create|ZIP|Include|Exclude|Comment|Swap|With|Data|Includes|Args|Output|Markdown|Log|Level|Info|Error|Warn|Sleep|Define|Function|Return|Declare|As|Global|Local|Verbose|Debug|Echo|Source|@Include|Accept|String|End-String|Delimited\s+By(?:\s+Size)?|(Up|Down)\s+By|Times|Time|Do|Exit|Cycle|Forever|Append|Prepend|Remove|Replace|Insert|At|Corresponding|Corr|If|Else|End-If|Exhibit|Add|Subtract|Giving|Multiply|Divide|Into|From|Compute|Equal|End-Compute|Display|Repeat|Perform|End-Perform|From|By|Unless|Until|Varying|For|Next|For-Each|Contains|Is|In|Not|Greater|Less|Than|Any|All|For|Next|Move|Evaluate|Sort|On|Key|File|Asc|Des|Unique|Printf|End|Otherwise|Assert|None|True|False|Nan|Zero|Inf|Colon|Comma|Newline|Period|Quote|Space|Tab|Choose|Print|Using|When|Matches|Unset|Reset|Set|To)(?:(?=$)|(?=[\s:;,]))", Keyword),
        ]
    }
