...
# Autognerated: maintained by bump_version.sh
...
class VGRCmdLine(CmdLine):
    ...
    def run(self):
        ...
        self._ctx.set_var_user((self._ctx.get_var_user(*DEFAULT_FOR_TYPE_PATH) or 'template-batch').lower(),
    ...
    def execute_statements(self, text: str) -> bool:
        try:
            self._ctx.execute_statements(text.rstrip(), '<shell>')
    ...
    @property
    def debug(self) -> bool:
        return self._ctx.dd.debug

    @property
    def verbose(self) -> bool:
        return self._ctx.dd.verbose

    @verbose.setter
    def verbose(self, value: bool):
        self._ctx.dd.verbose = value

    @property
    def prompt(self) -> str:
        return str(self._ctx.get_var(*SHELL_PROMPT_PATH) or self._DEFAULT_PROMPT)

    @prompt.setter
    def prompt(self, value: str):
        self._ctx.set_var(value or self._DEFAULT_PROMPT, *SHELL_PROMPT_PATH)

    @property
    def history_filename(self) -> str:
        return expand_filename(str(self._ctx.get_var(*SHELL_HISTORY_PATH) or self._DEFAULT_HISTORY))

    @history_filename.setter
    def history_filename(self, value: str) -> None:
        self._ctx.set_var(expand_filename(value or self._DEFAULT_HISTORY), *SHELL_HISTORY_PATH)

    @property
    def max_history_entries(self) -> int:
        try:
            return int(self._ctx.get_var(*SHELL_HISTORY_SIZE_PATH) or self._DEFAULT_HISTORY_SIZE)
        except ValueError:
            return self._DEFAULT_HISTORY_SIZE

    @max_history_entries.setter
    def max_history_entries(self, value: int) -> None:
        self._ctx.set_var(value or self._DEFAULT_HISTORY_SIZE, *SHELL_HISTORY_SIZE_PATH)
...
def load_extensions(dd: DataDictionary, extn_file: str) -> VgrExtensionRegistry:
    if dd.verbose: print_stderr('Loading extensions...')
...
def create_parser(dd: DataDictionary, grammar_file: str, extn_registry: VgrExtensionRegistry) -> Lark:
    if dd.verbose: print_stderr('Creating parser...')
    extn_from = ''
    extn_statements = ''
    extn_grammar = ''
...
"""
    ...
    extensions = load_extensions(dd, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'extensions.ini'))
    parser = create_parser(dd, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'vgr.ebnf'), extensions)
    ...
    try:
        ...
        if sys.stdin.isatty():
            ...
            if args.shell is True or (args.shell is None and not ordered_args):
                ctx.print_verbose('Starting the shell...')
                VGRCmdLine(ctx).run()
                ctx.print_verbose('Shell exited')
        else:
