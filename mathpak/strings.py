...
from .common import str_to_number, dist_list, dist_tuple
...
# TODO These dont work well when sub is a collection
...
def poly_append(x: Any, *args) -> Any: return reduce(_append, args, x)
def _append(x: Any, y: Any) -> Any:
    if x == None: return None if y == None else _append('', y)
    if y == None: return _append(x, '')
    ...
    if isinstance(x, (int, float)): return _append(str(x), y)
    if isinstance(y, (int, float)): return _append(x, str(y))
    if isinstance(x, list): return [_append(x1, y) for x1 in x]
    if isinstance(x, tuple): return (_append(x1, y) for x1 in x)
...
def poly_prepend(x: Any, *args) -> Any: return reduce(_prepend, args, x)
def _prepend(x: Any, y: Any) -> Any:
    if x == None: return None if y == None else _prepend('', y)
    if y == None: return _prepend(x, '')
    # NB: non-string like behavior
    ...
    if isinstance(x, (int, float)): return _prepend(str(x), y)
    if isinstance(y, (int, float)): return _prepend(x, str(y))
    if isinstance(x, list): return [_prepend(x1, y) for x1 in x]
    if isinstance(x, tuple): return (_prepend(x1, y) for x1 in x)
